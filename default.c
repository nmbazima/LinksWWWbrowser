...
static unsigned char *term_rd(struct option *o, unsigned char *c)
{
	...
	mem_free(w);
	if (!(w = get_token(&c))) goto err;
	if (strlen(cast_const_char w) != 1 || w[0] < '0' || w[0] > '5') goto err_f;
	ts->mode = w[0] - '0';
	mem_free(w);
	if (!(w = get_token(&c))) goto err;
	...
	ts->character_set = i;
	mem_free(w);
	r = get_token_num(&c, 0, 999, &ts->left_margin);
	if (r == -1) goto ret;
	if (r < 0) goto err;
	r = get_token_num(&c, 0, 999, &ts->right_margin);
	if (r < 0) goto err;
	r = get_token_num(&c, 0, 999, &ts->top_margin);
	if (r < 0) goto err;
	r = get_token_num(&c, 0, 999, &ts->bottom_margin);
	if (r < 0) goto err;
ret:
	return NULL;
err_f:
...
static unsigned char *dp_rd(struct option *o, unsigned char *c)
{
	...
	mem_free(cp);
	dp->kbd_codepage = cc;
	cc = get_token_num(&c, 0, 1, &dp->palette_mode);
	if (cc == -2) goto err;
	dp->nosave = 0;
	return NULL;
err:
...
static void dp_wr(struct option *o, unsigned char **s, int *l)
{
	...
	foreachback(struct driver_param, dp, ldp, driver_params) {
		if ((!dp->param || !*dp->param) && !*dp->shell_term && dp->kbd_codepage == -1 && !dp->palette_mode) continue;
